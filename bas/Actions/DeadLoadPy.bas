...
*if(x2>x1)
*format "%6d%8g%8g"
    ops.eleLoad('-ele', *ElemsNum, '-type', '-beamUniform', *operation(-DeadLoad*ctheta), *operation(-DeadLoad*stheta))
*else
*format "%6d%8g%8g"
    ops.eleLoad('-ele', *ElemsNum, '-type', '-beamUniform', *operation(DeadLoad*ctheta), *operation(-DeadLoad*stheta))
*endif
...
*if(strcmp(GenData(Vertical_axis),"Y")==0)
*format "%6d%8g"
    ops.load(*ElemsConec(1), 0.0, *operation(-DeadLoad), 0.0)
*format "%6d%8g"
    ops.load(*ElemsConec(2), 0.0, *operation(-DeadLoad), 0.0)
*else
*format "%6d%8g"
    ops.load(*ElemsConec(1), 0.0, 0.0, *operation(-DeadLoad))
*format "%6d%8g"
    ops.load(*ElemsConec(2), 0.0, 0.0, *operation(-DealLoad))
*endif
*else
*format "%6d%8g"
    ops.load(*ElemsConec(1), 0.0, *operation(-DeadLoad))
*format "%6d%8g"
    ops.load(*ElemsConec(2), 0.0, *operation(-DeadLoad))
*endif
*endif
*end elems
*endif
