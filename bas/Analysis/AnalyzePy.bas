...
*if(ndime==2)
    ops.recorder('Node', '-file', "Mode_{}.out".format(k), '-nodeRange', 1, *cntNodes, '-dof', 1, 2, "eigen", k)
*else
    ops.recorder('Node', '-file', "Mode_{}.out".format(k), '-nodeRange', 1, *cntNodes, '-dof', 1, 2, 3, "eigen", k)
*endif
}
...
T = []
for lam in lambda:
    T.append(6.283185/sqrt(lam))

# Write periods file
period = "Periods.out"
with open(period, "w") as file:
    for index, t in enumerate(T):
        file.write("{}  {}".format(index, t))
file.close()
...
if AnalOk == 0:
    print("Anlaysis completed SUCCESSFULLY")
else:
